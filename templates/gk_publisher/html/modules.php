	...
	if (!empty ($module->content)) {		
		...
		if($module->showtitle) {	
			if($params->get('module_link_switch')) {
				echo '<h3 class="header"><a href="'. $params->get('module_link') .'">'. preg_replace('/\*\*(.*?)\*\*/i', '<small>${1}</small>', $module->title) .'</a></h3>';
			} else {
				echo '<h3 class="header">'. preg_replace('/\*\*(.*?)\*\*/i', '<small>${1}</small>', $module->title) .'</h3>';
			}
		}
